...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['process_payment'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO payment_history (order_id, date, amount, status, method, delivery_option) VALUES (?, NOW(), ?, ?, ?, ?)");
        $stmt->bind_param("sdsss", $order_id, $amount, $status, $method, $delivery_option);
        $stmt->execute();
        $stmt->close();
        
        // Clear cart after successful payment
        $customer_id = 1; // This should be dynamically set based on logged-in user
...
// Fetch cart items (assuming customer_id = 1 for demonstration)
$customer_id = 1; // This should be dynamically set based on logged-in user
$cart_items = [];
$total_amount = 0;
$result = $conn->query("SELECT c.id, c.quantity, m.name, m.price, m.description 
                        FROM cart c 
...
<body>
    ...
    <div id="payment" class="tab-content active">
      <div class="card">
        ...
        <div class="cart-items">
          ...
          <?php else: ?>
            <?php foreach ($cart_items as $item): ?>
              <div class="cart-item">
                <div>
                  ...
                  <div style="font-size: 0.9rem; color: var(--gray);"><?php echo htmlspecialchars($item['description']); ?></div>
                  <div style="font-size: 0.9rem;">Quantity: <?php echo $item['quantity']; ?></div>
                </div>
            ...
            <div class="cart-total">
              Total: RM<?php echo number_format($total_amount, 2); ?>
            </div>
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" id="payment-amount" value="<?php echo number_format($total_amount, 2); ?>" readonly>
        ...
        <div class="form-group">
          ...
          <div class="error-message" id="delivery-option-error"></div>
        </div>
            ...
            <?php foreach ($payment_history as $payment): ?>
              <div class="history-item">
                <div>
                  ...
                  <div style="font-size: 0.9rem; color: var(--gray);"><?php echo ucfirst($payment['delivery_option']); ?></div>
                </div>
                <div style="text-align: right;">
  ...
  <script>
    ...
    function processPayment() {
      ...
      if (!deliveryOption) {
        document.getElementById('delivery-option-error').textContent = 'Please select an order type';
        document.getElementById('delivery-option-error').style.display = 'block';
        document.getElementById('delivery-option').classList.add('error');
        isValid = false;
      }
      ...
      .then(data => {
        if (data.status === 'success') {
          ...
          document.getElementById('receipt-delivery').textContent = deliveryOption.charAt(0).toUpperCase() + deliveryOption.slice(1);
